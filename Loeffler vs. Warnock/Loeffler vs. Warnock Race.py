# Voting Data Parser: Loeffler vs. Warnock
# This program parses the data from the November 3rd Special Election 
...
loefflerVsWarnock = Workbook("LoefflerVsWarnock.xlsx", {'strings_to_numbers': True})

# Only need to import data first time; webpage changes so I use less resources when I do it once and save

# Save URL

# url = "https://www.nytimes.com/interactive/2020/11/03/us/elections/results-georgia-senate-special.html?auth=login-google"
...
# Instead open premade file with correct data 

nyt_og_html = open("Loeffler vs. Warnock_HTML.txt", "r")
raw_html = nyt_og_html.read()

# raw_html = raw_html[167243:368377] only needed when original webpage imported

# Save data to these two files

nyt_html = open("Loeffler vs. Warnock_HTML.html", "w")
nyt_txt = open("Loeffler vs. Warnock_txt.txt", "w")
...
keep = ["name", "votes", "absentee_votes", "eevp", "eevp_value", "eevp_source", "absentee_max_ballots", "loefflerk", "warnockr", "collinsd", "taylork", "jacksond", "graysond", "jamesj", "liebermanm", "johnsonw", "johnson_shealeyt", "jacksona", "sladej", "winfieldr", "slowinskib", "buckleya", "bartella", "fortuinj", "greenem", "tarvere", "stovallv", "leader_margin_value", "leader_margin_display", "leader_margin_name_display", "leader_party_id", "results", "results_absentee", "write-ins"]
...
for i in range(len(county_data)):
    ...
    sheet = loefflerVsWarnock.add_worksheet(county_data[i][0][1])
    
    # Start the bar graph of in-person results with isolated data from each candidate
    
    results_chart = loefflerVsWarnock.add_chart({"type": "bar"})
    results_chart.add_series({"categories": [county_data[i][0][1], 0, 7, 0, 26], "values": [county_data[i][0][1], 1,  7, 1, 26]})
    ...
    sheet.insert_chart("H5", results_chart, {'x_scale': 1, 'y_scale': 2})
    
    # Start the bar graph of absentee results with isolated data from each candidate
    
    absentee_chart = loefflerVsWarnock.add_chart({"type": "bar"})
    absentee_chart.add_series({"categories": [county_data[i][0][1], 0, 27, 0, 46], "values": [county_data[i][0][1], 1, 27, 1, 46]})
    ...
    sheet.insert_chart("AB5", absentee_chart, {'x_scale': 1, 'y_scale': 2})
